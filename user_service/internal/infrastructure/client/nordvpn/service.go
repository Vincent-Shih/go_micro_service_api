...
//   - error: error
func (s *Service) GetLocation(ctx context.Context, ip string) (*GetLocationResponse, error) {
	...
	if err != nil {
		cus_otel.Error(ctx, "查詢ip地址", cus_otel.NewField("ip", ip), cus_otel.NewField("error", err))
		return result, err
	}
