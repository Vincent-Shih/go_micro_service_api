...
func (uq *UserQuery) QueryAuthClients() *AuthClientQuery {
	query := (&AuthClientClient{config: uq.config}).Query()
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withAuthClients:  uq.withAuthClients.Clone(),
...
func (uq *UserQuery) WithAuthClients(opts ...func(*AuthClientQuery)) *UserQuery {
	query := (&AuthClientClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withAuthClients = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	var (
		...
		_spec       = uq.querySpec()
		loadedTypes = [3]bool{
			uq.withAuthClients != nil,
			uq.withLoginRecords != nil,
			uq.withRoles != nil,
		}
	)
	if uq.withAuthClients != nil || uq.withRoles != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := uq.withAuthClients; query != nil {
		if err := uq.loadAuthClients(ctx, query, nodes, nil,
			func(n *User, e *AuthClient) { n.Edges.AuthClients = e }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadAuthClients(ctx context.Context, query *AuthClientQuery, nodes []*User, init func(*User), assign func(*User, *AuthClient)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*User)
	for i := range nodes {
		if nodes[i].auth_client_users == nil {
			continue
		}
		fk := *nodes[i].auth_client_users
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "auth_client_users" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (uq *UserQuery) loadLoginRecords(ctx context.Context, query *LoginRecordQuery, nodes []*User, init func(*User), assign func(*User, *LoginRecord)) error {
	...
	for _, n := range neighbors {
		fk := n.user_login_records
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_login_records" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_login_records" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (uq *UserQuery) loadRoles(ctx context.Context, query *RoleQuery, nodes []*User, init func(*User), assign func(*User, *Role)) error {
	...
	nodeids := make(map[int64][]*User)
	for i := range nodes {
		if nodes[i].user_roles == nil {
			continue
		}
		fk := *nodes[i].user_roles
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_roles" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
