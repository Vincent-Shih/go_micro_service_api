...
import (
	...
	"go_micro_service_api/auth_service/internal/infrastructure/ent_impl/ent/predicate"
...
func (rq *RoleQuery) QueryAuthClients() *AuthClientQuery {
	query := (&AuthClientClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *RoleQuery) Clone() *RoleQuery {
	...
	return &RoleQuery{
		...
		withAuthClients: rq.withAuthClients.Clone(),
...
func (rq *RoleQuery) WithAuthClients(opts ...func(*AuthClientQuery)) *RoleQuery {
	query := (&AuthClientClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withAuthClients = query
	return rq
}
...
func (rq *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	if query := rq.withAuthClients; query != nil {
		if err := rq.loadAuthClients(ctx, query, nodes,
			func(n *Role) { n.Edges.AuthClients = []*AuthClient{} },
			func(n *Role, e *AuthClient) { n.Edges.AuthClients = append(n.Edges.AuthClients, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *RoleQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*Role, init func(*Role), assign func(*Role, *User)) error {
	...
	for _, n := range neighbors {
		fk := n.user_roles
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_roles" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_roles" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (rq *RoleQuery) loadAuthClients(ctx context.Context, query *AuthClientQuery, nodes []*Role, init func(*Role), assign func(*Role, *AuthClient)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(role.AuthClientsTable)
		s.Join(joinT).On(s.C(authclient.FieldID), joinT.C(role.AuthClientsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(role.AuthClientsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(role.AuthClientsPrimaryKey[1]))
	...
	neighbors, err := withInterceptors[[]*AuthClient](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "auth_clients" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
