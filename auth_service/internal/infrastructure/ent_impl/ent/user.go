...
type User struct {
	...
	// PasswordFailTimes holds the value of the "password_fail_times" field.
	PasswordFailTimes int `json:"password_fail_times,omitempty"`
	// Status holds the value of the "status" field.
	Status int `json:"status,omitempty"`
	// Edges holds the relations/edges for other nodes in the graph.
	// The values are being populated by the UserQuery when eager-loading is set.
	Edges             UserEdges `json:"edges"`
	auth_client_users *int64
	user_roles        *int64
...
func (e UserEdges) AuthClientsOrErr() (*AuthClient, error) {
...
func (*User) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case user.FieldID, user.FieldPasswordFailTimes, user.FieldStatus:
			values[i] = new(sql.NullInt64)
		case user.FieldAccount, user.FieldPassword:
			values[i] = new(sql.NullString)
		case user.FieldCreatedAt, user.FieldUpdatedAt:
			values[i] = new(sql.NullTime)
		case user.ForeignKeys[0]: // auth_client_users
			values[i] = new(sql.NullInt64)
		case user.ForeignKeys[1]: // user_roles
			values[i] = new(sql.NullInt64)
		default:
	...
	for i := range columns {
		...
		case user.FieldPasswordFailTimes:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field password_fail_times", values[i])
			} else if value.Valid {
				u.PasswordFailTimes = int(value.Int64)
			}
		...
		case user.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field auth_client_users", value)
			} else if value.Valid {
				u.auth_client_users = new(int64)
				*u.auth_client_users = int64(value.Int64)
			}
		case user.ForeignKeys[1]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field user_roles", value)
			} else if value.Valid {
				u.user_roles = new(int64)
				*u.user_roles = int64(value.Int64)
			}
		default:
...
// QueryAuthClients queries the "auth_clients" edge of the User entity.
func (u *User) QueryAuthClients() *AuthClientQuery {
	return NewUserClient(u.config).QueryAuthClients(u)
}
...
func (u *User) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("password_fail_times=")
	builder.WriteString(fmt.Sprintf("%v", u.PasswordFailTimes))
	builder.WriteString(", ")
