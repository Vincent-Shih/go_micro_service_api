...
func (r *RedisCache) Get(ctx context.Context, key string) (string, *cus_err.CusError) {
	...
	if err != nil {
		...
		kgsErr := cus_err.New(errCode, "Failed to get key", err)
...
func (r *RedisCache) Set(ctx context.Context, key string, value string, expiration time.Duration) *cus_err.CusError {
	...
	if err != nil {
		...
		kgsErr := cus_err.New(cus_err.InternalServerError, "Failed to set key", err)
		cus_otel.Error(ctx, kgsErr.Error())
		return kgsErr
	}
...
func (r *RedisCache) GetObject(ctx context.Context, key string, dest any) *cus_err.CusError {
	...
	if err != nil {
		// When key not found return ResourceNotFound error.
		if err == redis.Nil {
			kgsErr := cus_err.New(cus_err.ResourceNotFound, "Failed to get key", err)
			cus_otel.Error(ctx, kgsErr.Error())
			return kgsErr
		}
		// Otherwise, return InternalServerError error.
		kgsErr := cus_err.New(cus_err.InternalServerError, "Failed to get key", err)
		cus_otel.Error(ctx, kgsErr.Error())
		return kgsErr
	}
	...
	if err != nil {
		// Return InternalServerError error.
		kgsErr := cus_err.New(cus_err.InternalServerError, "Failed to unmarshal value", err)
		cus_otel.Error(ctx, kgsErr.Error())
		return kgsErr
	}
...
func (r *RedisCache) SetObject(ctx context.Context, key string, value any, expiration time.Duration) *cus_err.CusError {
	...
	if err != nil {
		// Return InternalServerError error.
		kgsErr := cus_err.New(cus_err.InternalServerError, "Failed to marshal value", err)
		cus_otel.Error(ctx, kgsErr.Error())
		return kgsErr
	}
...
func (r *RedisCache) Delete(ctx context.Context, keys ...string) *cus_err.CusError {
	...
	if err != nil {
		// Return InternalServerError error.
		kgsErr := cus_err.New(cus_err.InternalServerError, "Failed to delete key", err)
		cus_otel.Error(ctx, kgsErr.Error())
		return kgsErr
	}
	...
	if result == 0 {
		kgsErr := cus_err.New(cus_err.ResourceNotFound, "Key not found", nil)
		cus_otel.Error(ctx, kgsErr.Error())
		return kgsErr
	}
...
func (r *RedisCache) GetHash(ctx context.Context, key string, dest any) *cus_err.CusError {
	...
	if err := res.Err(); err != nil {
		...
		kgsErr := cus_err.New(errCode, "Failed to get hash", err)
		cus_otel.Error(ctx, kgsErr.Error())
		return kgsErr
	}

	if err := res.Scan(dest); err != nil {
		kgsErr := cus_err.New(cus_err.InternalServerError, "Failed to scan hash", err)
		cus_otel.Error(ctx, kgsErr.Error())
		return kgsErr
	}
...
func (r *RedisCache) GetHashFields(ctx context.Context, key string, fields ...string) ([]interface{}, *cus_err.CusError) {
	...
	if err != nil {
		...
		kgsErr := cus_err.New(errCode, "Failed to get hash", err)
		cus_otel.Error(ctx, kgsErr.Error())
		return make([]interface{}, 0), kgsErr
	}
...
func (r *RedisCache) SetHash(ctx context.Context, expiration time.Duration, key string, values ...interface{}) *cus_err.CusError {
	...
	if err := r.client.HSet(ctx, key, values...).Err(); err != nil {
		kgsErr := cus_err.New(cus_err.InternalServerError, "Failed to set hash", err)
		cus_otel.Error(ctx, kgsErr.Error())
		return kgsErr
	}
	...
	if expiration > 0 {
		if err := r.client.Expire(ctx, key, expiration).Err(); err != nil {
			kgsErr := cus_err.New(cus_err.InternalServerError, "Failed to set expiration", err)
			cus_otel.Error(ctx, kgsErr.Error())
			return kgsErr
		}
	}
...
func (r *RedisCache) Incr(ctx context.Context, key string) (int64, *cus_err.CusError) {
	...
	if err != nil {
		...
		kgsErr := cus_err.New(errCode, "Failed to increment", err)
		cus_otel.Error(ctx, kgsErr.Error())
		return 0, kgsErr
	}
